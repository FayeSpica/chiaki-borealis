...
bool IO::ReadGameKeys(SDL_Event *event, ChiakiControllerState *state)
{
	...
	switch(event->type)
	{
            ...
            switch (event->caxis.axis) {
                case SDL_CONTROLLER_AXIS_TRIGGERLEFT:
                    if (event->caxis.value > 0) {
                        state->l2_state = 0xff & (event->caxis.value >> 7);
                        CHIAKI_LOGI(this->log, "Left trigger pressed: %d, state: %d\n", event->caxis.value, state->l2_state);
                    } else {
                        CHIAKI_LOGI(this->log, "Left trigger released\n");
                        state->l2_state = 0;
                    }
                    break;
                case SDL_CONTROLLER_AXIS_TRIGGERRIGHT:
                    if (event->caxis.value > 0) {
                        state->r2_state = 0xff & (event->caxis.value >> 7);
                        CHIAKI_LOGI(this->log, "Right trigger pressed: %d, state: %d\n", event->caxis.value, state->r2_state);
                    } else {
                        CHIAKI_LOGI(this->log, "Right trigger released\n");
                        state->r2_state = 0;
                    }
                    break;
		...
		case SDL_CONTROLLERBUTTONUP:
			CHIAKI_LOGI(this->log, "Controller %d button %d UP\n", event->cbutton.which, event->cbutton.button);
            switch(event->cbutton.button) {
                case 0:
                    state->buttons ^= CHIAKI_CONTROLLER_BUTTON_CROSS;
                    break;
        ...
        case SDL_JOYBUTTONDOWN:
            CHIAKI_LOGI(this->log, "Joy %d button %d DOWN\n", event->jbutton.which, event->jbutton.button);
        ...
        case SDL_JOYBUTTONUP:
            CHIAKI_LOGI(this->log, "Joy %d button %d UP\n", event->jbutton.which, event->jbutton.button);
            switch(event->jbutton.button) {
                case 11:
                    state->buttons ^= CHIAKI_CONTROLLER_BUTTON_SHARE;
                    break;
...
bool IO::InitController()
{
	...
	for(int i = 0; i < SDL_JOYSTICK_COUNT && i < SDL_NumJoysticks(); i++)
	{
        if (!SDL_IsGameController(i)) {
            CHIAKI_LOGE(this->log, "SDL_GameControllerOpen: %s IsGameController=false\n", SDL_GetError());
            continue;
        }
		this->sdl_joystick_ptr[i] = SDL_GameControllerOpen(i);
		if(sdl_joystick_ptr[i] == nullptr)
		{
			CHIAKI_LOGE(this->log, "SDL_GameControllerOpen: %s\n", SDL_GetError());
            continue;
		}
...
bool IO::FreeController()
{
	for(int i = 0; i < SDL_JOYSTICK_COUNT && i < SDL_NumJoysticks(); i++)
	{
		SDL_GameControllerClose(this->sdl_joystick_ptr[i]);
